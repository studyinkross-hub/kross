...
import {segmentTypes,type SegmentType} from '@/lib/course-flow';
...
type Video={id:string;title:string;level:string;lessonNumber:number;lessonTitle:string;videoUrl:string;duration:number;segmentType:SegmentType};
...
export async function GET(){
  try{
    ...
    const videos=r.results.map(x=>JSON.parse(x.payload) as Partial<Video>).map((video,index)=>({...video,lessonNumber:Number(video.lessonNumber)||1,lessonTitle:video.lessonTitle||video.title||`Lesson ${index+1}`,segmentType:segmentTypes.has(video.segmentType as SegmentType)?video.segmentType:'custom'}));
...
export async function POST(req:Request){
  ...
  try{
    ...
    if(typeof b.title!=='string'||!b.title.trim()||b.title.length>120||typeof b.lessonTitle!=='string'||!b.lessonTitle.trim()||b.lessonTitle.length>120||!Number.isInteger(b.lessonNumber)||b.lessonNumber<1||b.lessonNumber>999||!levels.includes(b.level)||!segmentTypes.has(b.segmentType)||!internal||!Number.isInteger(b.duration)||b.duration<10||b.duration>14400)return Response.json({error:'INVALID'},{status:400});
    const item:Video={id:crypto.randomUUID(),title:b.title.trim(),level:b.level,lessonNumber:b.lessonNumber,lessonTitle:b.lessonTitle.trim(),videoUrl:b.videoUrl,duration:b.duration,segmentType:b.segmentType};
